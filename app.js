// app.js — Factory Labour Logger (full UI, no build tools)
// Requires index.html to load React, ReactDOM, and Tailwind via CDN BEFORE this file.
...
function roundToIncrement(mins, inc){ return Math.round(mins / Math.max(1, inc)) * Math.max(1, inc); }
...
// --- Small subcomponents (using React.createElement) ---
  ...
  return React.createElement("div", { className: "mt-2" },
    !open
      ...
      : React.createElement("div", { className: "flex gap-2 mt-1" },
          ...
          React.createElement("button", { className: "px-3 rounded-xl border border-slate-300 hover:bg-slate-50", onClick: () => { onAdd(val); setVal(""); setOpen(false); } }, "Add"),
...
function QuickAddProject({ onAdd }) {
  ...
  return React.createElement("div", { className: "mt-2" },
    !open
      ...
      : React.createElement("div", { className: "grid grid-cols-5 gap-2 mt-1 items-center" },
          ...
          React.createElement("div", { className: "col-span-5 flex gap-2" },
            React.createElement("button", { className: "px-3 rounded-xl border border-slate-300 hover:bg-slate-50", onClick: () => { onAdd(code, name); setCode(""); setName(""); setOpen(false); } }, "Add"),
            React.createElement("button", { className: "px-3 rounded-xl border border-slate-200 text-slate-500 hover:bg-slate-50", onClick: () => setOpen(false) }, "Cancel")
          )
        )
  );
...
function App(){
  ...
  function resetForm(){ setProjectId(""); setBarcode(""); setWorkType("Fabrication"); setNotes(""); setManualHours(""); setBreakMin(30); setStart("08:00"); setEnd("16:30"); setError(""); setOkMsg(""); }
  function addEmployee(name){ const n=name?.trim(); if(!n) return; if(employees.some(e=>e.name.toLowerCase()===n.toLowerCase())) return; setEmployees([...employees,{id:uid(),name:n}]); }
  function addProject(code,name){ const c=code?.trim(), n=name?.trim(); if(!c||!n) return; if(projects.some(p=>p.code.toLowerCase()===c.toLowerCase())) return; setProjects([...projects,{id:uid(),code:c,name:n}]); }
  ...
  function importProjectsCsv(file){
    ...
    r.onload=()=>{ const text=r.result?.toString()||""; const rows=text.split(/\r?\n/).filter(Boolean);
      const [h,...data]=rows; const headers=h.split(",").map(s=>s.trim().toLowerCase());
      const ci=headers.indexOf("code"), ni=headers.indexOf("name");
      if(ci===-1||ni===-1) return alert("CSV must have headers: code,name");
      const add=[]; for(const row of data){ const cols=parseCsvRow(row); const code=(cols[ci]||"").trim(); const name=(cols[ni]||"").trim(); if(!code||!name) continue; if(!projects.some(p=>p.code.toLowerCase()===code.toLowerCase())) add.push({id:uid(),code,name}); }
      if(add.length) setProjects(prev=>[...prev,...add]); alert(`Imported ${add.length} new projects`);
    };
    r.readAsText(file);
  }
  ...
  function handleBarcodeEnter(e){
    if(e.key==="Enter"){
      ...
      if(p){ setProjectId(p.id); setOkMsg(`Project set: ${p.code}`); } else setError(`No project with code ${code}`);
      setBarcode("");
    }
  }
  ...
  function addEntry(){
    ...
    if(manualHours===""){ if(toMinutes(end)<=toMinutes(start)) return setError("End time must be after start time"); if(breakMin<0||breakMin>240) return setError("Break minutes should be between 0 and 240"); }
    const entry={ id:uid(), employeeId, projectId, date, start, end, breakMin:Number(breakMin)||0, workType, notes:notes.trim(), hours:+hrs.toFixed(2), roundedFromMin:computed.rawMins, roundingMin, status:"pending", locked:false, createdAt:new Date().toISOString() };
    setEntries(prev=>[entry,...prev]); setOkMsg("Entry added"); resetForm();
  }
  function deleteEntry(id){ const e=entries.find(x=>x.id===id); if(e?.locked) return alert("Approved entries are locked."); setEntries(prev=>prev.filter(e=>e.id!==id)); }
  function setStatus(id,status){ setEntries(prev=> prev.map(e=> e.id===id? {...e,status,locked: status==="approved" ? true : e.locked && status==="approved"} : e )); }
  ...
  const withOt = useMemo(()=>{
    const by=new Map(); for(const e of visibleEntries){ const k=`${e.employeeId}|${e.date}`; if(!by.has(k)) by.set(k,[]); by.get(k).push(e); }
    const res=new Map();
  ...
  const report = useMemo(()=>{
    ...
    for(const e of rows){
      ...
      const er=byEmp.get(emp)||{hours:0,base:0,ot:0}; er.hours+=(e.hours||0); er.base+=alloc.base; er.ot+=alloc.ot; byEmp.set(emp,er);
      const pr=byProj.get(proj)||{hours:0,base:0,ot:0}; pr.hours+=(e.hours||0); pr.base+=alloc.base; pr.ot+=alloc.ot; byProj.set(proj,pr);
    }
  ...
  function downloadCSV(){
    const csv=toCSV(visibleEntries); const blob=new Blob([csv],{type:"text/csv;charset=utf-8;"}); const url=URL.createObjectURL(blob);
    const a=document.createElement("a"); a.href=url; a.download=`labour-hours-${new Date().toISOString().slice(0,10)}.csv`; a.click(); URL.revokeObjectURL(url);
  }
  ...
  async function exportXLSX(){
    try{
      ...
      XLSX.utils.book_append_sheet(wb,s1,"By Employee"); XLSX.utils.book_append_sheet(wb,s2,"By Project");
  ...
  async function exportPDF(){
    try{
      ...
      const doc=new jsPDF(); doc.setFontSize(14); doc.text(`Labour Report — Week ${reportWeek}`,14,16); doc.setFontSize(10); doc.text(`Generated: ${new Date().toLocaleString()}`,14,22);
      ...
      const today=new Date().toISOString().slice(0,10); doc.save(`labour-report-${reportWeek}-${today}.pdf`);
    }catch(e){ alert("PDF export failed: "+e.message); }
  }

  async function apiFetch(path, options={}){
    const headers={ 'Content-Type':'application/json', ...(auth.token?{Authorization:`Bearer ${auth.token}`}:{}) };
    const res=await fetch(apiBase.replace(/\/+$/,'')+path, {...options, headers:{...headers, ...(options.headers||{})}});
  ...
  async function login(){
    setSyncMsg("");
    try{
      const body=new URLSearchParams(); body.set("username",loginEmail); body.set("password",loginPass);
      const r=await fetch(apiBase.replace(/\/+$/,"")+"/auth/login",{method:"POST", body});
      if(!r.ok) throw new Error((await r.text())||"Login error");
      const data=await r.json(); setAuth({user:data.user, token:data.token}); setShowLogin(false); setSyncMsg("Signed in ✔");
  ...
  async function syncNow(){
    ...
    setSyncMsg("Syncing... (demo)");
    setTimeout(()=> setSyncMsg("Synced ✔"), 600);
  }
  ...
  function approveWeek(){
    ...
    if("Notification" in window && Notification.permission==="granted") new Notification("Supervisor update",{ body:`Week ${target} approved` });
  }
  function rejectWeek(){
    const target=weekIso;
    setEntries(prev=> prev.map(e=> isoWeekId(new Date(e.date))===target ? {...e,status:"rejected",locked:false} : e));
    if("Notification" in window && Notification.permission==="granted") new Notification("Supervisor update",{ body:`Week ${target} rejected` });
  }
  ...
  return React.createElement("div",{className:"min-h-screen"},
    // Header
    React.createElement("header",{className:"sticky top-0 z-10 bg-white/80 backdrop-blur border-b border-slate-200"},
    ...
    // Login modal
    showLogin && React.createElement("div",{className:"fixed inset-0 bg-black/30 flex items-center justify-center p-4"},
    ...
    // Main sections
    React.createElement("main",{className:"max-w-6xl mx-auto px-4 py-6 space-y-6"},

      // Settings & Cloud
      React.createElement("section",{className:"bg-white rounded-2xl shadow p-4 md:p-6"},
        ...
        React.createElement("div",{className:"grid md:grid-cols-8 gap-3 items-end"},
          React.createElement("div",null, React.createElement("label",{className:"block text-sm font-medium mb-1"},"Rounding (minutes)"), React.createElement("input",{type:"number",min:1,className:"w-full rounded-xl border-slate-300 focus:ring-2 focus:ring-slate-400",value:roundingMin,onChange:e=>setRoundingMin(Math.max(1,Number(e.target.value)))})),
          React.createElement("div",null, React.createElement("label",{className:"block text-sm font-medium mb-1"},"Daily OT threshold (hours)"), React.createElement("input",{type:"number",min:0,step:"0.25",className:"w-full rounded-xl border-slate-300 focus:ring-2 focus:ring-slate-400",value:otDailyThreshold,onChange:e=>setOtDailyThreshold(Math.max(0,Number(e.target.value)))})),
          React.createElement("div",{className:"md:col-span-3"}, React.createElement("label",{className:"block text-sm font-medium mb-1"},"API base URL"), React.createElement("input",{type:"text",className:"w-full rounded-xl border-slate-300 focus:ring-2 focus:ring-slate-400",value:apiBase,onChange:e=>setApiBase(e.target.value)})),
          React.createElement("div",{className:"flex items-end gap-2"}, React.createElement("button",{onClick:syncNow,className:"px-3 py-2 rounded-xl border border-slate-300 hover:bg-slate-50"},"Sync now"), React.createElement("button",{onClick:()=>setKiosk(v=>!v),className:"px-3 py-2 rounded-xl bg-slate-900 text-white hover:bg-slate-800"}, kiosk?"Exit kiosk":"Enter kiosk"))
        ),
        syncMsg && React.createElement("div",{className:"text-xs text-slate-600 mt-2"}, syncMsg)
      ),

      // Kiosk
      kiosk && React.createElement("section",{className:"bg-white rounded-2xl shadow p-4 md:p-6"},
        ...
        React.createElement("div",{className:"grid md:grid-cols-3 gap-3 items-end"},
          React.createElement("div",null,
            React.createElement("label",{className:"block text-sm font-medium mb-1"},"Employee PIN / Badge / Name"),
            ...
            React.createElement("button",{className:"mt-2 px-3 py-2 rounded-xl border border-slate-300 hover:bg-slate-50",onClick:()=>{ const v=kioskPin.trim().toLowerCase(); const emp=employees.find(e=> e.name.toLowerCase()===v || e.id.toLowerCase()===v || (e.badge||'').toLowerCase()===v || (e.pin||'').toLowerCase()===v ); if(emp) setKioskEmployeeId(emp.id); else alert("No matching employee"); }},"Select employee"),
            kioskEmployeeId && React.createElement("div",{className:"text-sm text-slate-600 mt-1"},"Selected: ", React.createElement("span",{className:"font-semibold"}, employeeById[kioskEmployeeId]?.name))
          ),
          React.createElement("div",null,
            React.createElement("label",{className:"block text-sm font-medium mb-1"},"Project (scan or pick)"),
          ...
          React.createElement("div",{className:"flex gap-2 items-end"},
            !kioskRunning
              ...
              : React.createElement("button",{className:"w-full px-4 py-6 rounded-2xl bg-red-600 text-white text-xl",onClick:()=>{ const startISO=kioskRunning.startISO; const startDt=new Date(startISO); const endDt=new Date(); const mins=Math.max(0,Math.round((endDt-startDt)/60000)); const rounded= roundToIncrement(Math.max(0, mins-(Number(breakMin)||0)), Math.max(1, roundingMin)); const hrs= +(rounded/60).toFixed(2); const entry={ id:uid(), employeeId:kioskEmployeeId, projectId:kioskProjectId, date:new Date().toISOString().slice(0,10), start: fromMinutes(toMinutes(`${String(startDt.getHours()).padStart(2,'0')}:${String(startDt.getMinutes()).padStart(2,'0')}`)), end: fromMinutes(toMinutes(`${String(endDt.getHours()).padStart(2,'0')}:${String(endDt.getMinutes()).padStart(2,'0')}`)), breakMin:Number(breakMin)||0, workType:"Kiosk", notes:"", hours:hrs, roundedFromMin:mins, roundingMin, status:"pending", locked:false, createdAt:new Date().toISOString() }; setEntries(prev=>[entry,...prev]); setKioskRunning(null); }},"Stop")
          )
        ),
      ...
      React.createElement("section",{className:"bg-white rounded-2xl shadow p-4 md:p-6"},
        ...
        React.createElement("div",{className:"grid md:grid-cols-4 gap-3"},
          React.createElement("div",null, React.createElement("label",{className:"block text-sm font-medium mb-1"},"Employee"),
            React.createElement("select",{className:"w-full rounded-xl border-slate-300 focus:ring-2 focus:ring-slate-400",value:employeeId,onChange:e=>setEmployeeId(e.target.value)},
              React.createElement("option",{value:""},"Select..."), employees.map(e=> React.createElement("option",{key:e.id,value:e.id}, e.name))
            ),
            React.createElement(QuickAdd,{onAdd:addEmployee,label:"Add employee",placeholder:"e.g. David"})
          ),
          React.createElement("div",null, React.createElement("label",{className:"block text-sm font-medium mb-1"},"Project"),
            React.createElement("select",{className:"w-full rounded-xl border-slate-300 focus:ring-2 focus:ring-slate-400",value:projectId,onChange:e=>setProjectId(e.target.value)},
              React.createElement("option",{value:""},"Select..."), projects.map(p=> React.createElement("option",{key:p.id,value:p.id}, `${p.code} — ${p.name}`))
            ),
          ...
          React.createElement("div",null, React.createElement("label",{className:"block text-sm font-medium mb-1"},"Date"), React.createElement("input",{type:"date",className:"w-full rounded-xl border-slate-300 focus:ring-2 focus:ring-slate-400",value:date,onChange:e=>setDate(e.target.value)})),
          React.createElement("div",null, React.createElement("label",{className:"block text-sm font-medium mb-1"},"Work Type"), React.createElement("input",{type:"text",className:"w-full rounded-xl border-slate-300 focus:ring-2 focus:ring-slate-400",value:workType,onChange:e=>setWorkType(e.target.value),placeholder:"e.g. Fabrication, Assembly, QA"})),
          React.createElement("div",null, React.createElement("label",{className:"block text-sm font-medium mb-1"},"Start"), React.createElement("input",{type:"time",className:"w-full rounded-xl border-slate-300 focus:ring-2 focus:ring-slate-400",value:start,onChange:e=>setStart(e.target.value)})),
          React.createElement("div",null, React.createElement("label",{className:"block text-sm font-medium mb-1"},"End"), React.createElement("input",{type:"time",className:"w-full rounded-xl border-slate-300 focus:ring-2 focus:ring-slate-400",value:end,onChange:e=>setEnd(e.target.value)})),
          React.createElement("div",null, React.createElement("label",{className:"block text-sm font-medium mb-1"},"Break (minutes)"), React.createElement("input",{type:"number",min:0,max:240,className:"w-full rounded-xl border-slate-300 focus:ring-2 focus:ring-slate-400",value:breakMin,onChange:e=>setBreakMin(Number(e.target.value))})),
          React.createElement("div",null, React.createElement("label",{className:"block text-sm font-medium mb-1"},"Manual Hours (optional)"), React.createElement("input",{type:"number",step:"0.01",className:"w-full rounded-xl border-slate-300 focus:ring-2 focus:ring-slate-400",value:manualHours,onChange:e=>setManualHours(e.target.value),placeholder:"Override computed hours"}), React.createElement("div",{className:"text-xs text-slate-500 mt-1"},"Computed (rounded): ", React.createElement("span",{className:"font-semibold"}, (+(computed.hours||0)).toFixed(2)), " h"))
        ),
        React.createElement("div",{className:"mt-4 flex items-center justify-between"},
          React.createElement("div",{className:"text-sm text-slate-600"},"Rounded from ", Math.max(0, computed.rawMins), " mins → ", React.createElement("span",{className:"font-semibold"}, (+(computed.hours||0)).toFixed(2), " h")),
      ...
      React.createElement("section",{className:"bg-white rounded-2xl shadow p-4 md:p-6"},
        React.createElement("div",{className:"grid md:grid-cols-6 gap-3 items-end"},
          ...
          React.createElement("div",null, React.createElement("label",{className:"block text-sm font-medium mb-1"},"From"), React.createElement("input",{type:"date",className:"w-full rounded-xl border-slate-300 focus:ring-2 focus:ring-slate-400",value:fromDate,onChange:e=>setFromDate(e.target.value)})),
          React.createElement("div",null, React.createElement("label",{className:"block text-sm font-medium mb-1"},"To"), React.createElement("input",{type:"date",className:"w-full rounded-xl border-slate-300 focus:ring-2 focus:ring-slate-400",value:toDate,onChange:e=>setToDate(e.target.value)})),
          React.createElement("div",null, React.createElement("label",{className:"block text-sm font-medium mb-1"},"Import projects (CSV)"), React.createElement("input",{type:"file",accept:".csv",className:"w-full rounded-xl border-slate-300 focus:ring-2 focus:ring-slate-400",onChange:e=> e.target.files?.[0] && importProjectsCsv(e.target.files[0])}))
        ),
        React.createElement("div",{className:"mt-4 flex items-center justify-between flex-wrap gap-2"},
          React.createElement("div",{className:"text-sm"},"Total (filtered): ", React.createElement("span",{className:"font-semibold"}, totals.totalHours.toFixed(2)), " h • Base: ", React.createElement("span",{className:"font-semibold"}, totals.totalBase.toFixed(2)), " • OT: ", React.createElement("span",{className:"font-semibold"}, totals.totalOt.toFixed(2))),
          React.createElement("div",{className:"flex gap-2"},
        ...
        React.createElement("div",{className:"mt-4 grid md:grid-cols-2 gap-4"},
          React.createElement("div",{className:"rounded-xl border border-slate-200 p-3"},
            React.createElement("h3",{className:"font-medium mb-2"},"Hours by Project"),
            React.createElement("ul",{className:"space-y-1 text-sm"},
              Object.entries(totals.byProject).length===0 && React.createElement("li",{className:"text-slate-500"},"No data"),
              Object.entries(totals.byProject).map(([pid, hrs]) =>
                React.createElement("li",{key:pid,className:"flex justify-between"},
                  React.createElement("span",null, projectById[pid]?.code," — ",projectById[pid]?.name),
                  React.createElement("span",{className:"font-semibold"}, hrs.toFixed(2))
                )
              )
            )
          ),
          React.createElement("div",{className:"rounded-xl border border-slate-200 p-3"},
            React.createElement("h3",{className:"font-medium mb-2"},"Hours by Employee"),
            React.createElement("ul",{className:"space-y-1 text-sm"},
              Object.entries(totals.byEmployee).length===0 && React.createElement("li",{className:"text-slate-500"},"No data"),
              Object.entries(totals.byEmployee).map(([eid, hrs]) =>
                React.createElement("li",{key:eid,className:"flex justify-between"},
                  React.createElement("span",null, employeeById[eid]?.name),
                  React.createElement("span",{className:"font-semibold"}, hrs.toFixed(2))
                )
              )
            )
        ...
        React.createElement("div",{className:"grid md:grid-cols-2 gap-4"},
          React.createElement("div",{className:"rounded-xl border border-slate-200 p-3"},
            ...
            React.createElement("table",{className:"w-full text-sm"},
              React.createElement("thead",null, React.createElement("tr",{className:"text-left text-slate-600 border-b"}, React.createElement("th",{className:"py-1"},"Employee"), React.createElement("th",{className:"py-1"},"Hours"), React.createElement("th",{className:"py-1"},"Base"), React.createElement("th",{className:"py-1"},"OT"))),
              React.createElement("tbody",null, report.byEmp.length===0 && React.createElement("tr",null, React.createElement("td",{colSpan:4,className:"py-3 text-slate-500"},"No data")),
                report.byEmp.map(r=> React.createElement("tr",{key:r.key,className:"border-b last:border-b-0"}, React.createElement("td",{className:"py-1"},r.key), React.createElement("td",{className:"py-1"},r.hours.toFixed(2)), React.createElement("td",{className:"py-1"},r.base.toFixed(2)), React.createElement("td",{className:"py-1"},r.ot.toFixed(2)))))
          ),
          React.createElement("div",{className:"rounded-xl border border-slate-200 p-3"},
            React.createElement("h3",{className:"font-medium mb-2"},"By project (week ", reportWeek, ")"),
            React.createElement("table",{className:"w-full text-sm"},
              React.createElement("thead",null, React.createElement("tr",{className:"text-left text-slate-600 border-b"}, React.createElement("th",{className:"py-1"},"Project"), React.createElement("th",{className:"py-1"},"Hours"), React.createElement("th",{className:"py-1"},"Base"), React.createElement("th",{className:"py-1"},"OT"))),
              React.createElement("tbody",null, report.byProj.length===0 && React.createElement("tr",null, React.createElement("td",{colSpan:4,className:"py-3 text-slate-500"},"No data")),
                report.byProj.map(r=> React.createElement("tr",{key:r.key,className:"border-b last:border-b-0"}, React.createElement("td",{className:"py-1"},r.key), React.createElement("td",{className:"py-1"},r.hours.toFixed(2)), React.createElement("td",{className:"py-1"},r.base.toFixed(2)), React.createElement("td",{className:"py-1"},r.ot.toFixed(2)))))
          )
        ),
        React.createElement("div",{className:"mt-3 flex gap-2"},
          React.createElement("button",{onClick:exportXLSX,className:"px-3 py-2 rounded-xl border border-slate-300 hover:bg-slate-50"},"Export XLSX"),
          React.createElement("button",{onClick:exportPDF,className:"px-3 py-2 rounded-xl border border-slate-300 hover:bg-slate-50"},"Export PDF")
        )
      ),
        ...
        React.createElement("div",{className:"overflow-x-auto"},
          React.createElement("table",{className:"min-w-full text-sm"},
            ...
            React.createElement("tbody",null,
              visibleEntries.length===0 && React.createElement("tr",null, React.createElement("td",{colSpan:13,className:"py-6 text-center text-slate-500"},"No entries yet. Use the form above to add your first one.")),
              visibleEntries.map(e=>{
                ...
                return React.createElement("tr",{key:e.id,className:"border-b last:border-b-0"},
                  ...
                  React.createElement("td",{className:"py-2 pr-3 whitespace-nowrap"},
                    React.createElement("span",{className:`px-2 py-0.5 rounded-lg text-xs ${e.status==="approved"?"bg-emerald-100 text-emerald-800":e.status==="rejected"?"bg-red-100 text-red-800":"bg-slate-100 text-slate-700"}`}, e.status),
                    e.locked && React.createElement("span",{className:"ml-2 text-xs text-slate-500"},"(locked)")
                  ),
      ...
      // Notes
      React.createElement("details",{className:"bg-white rounded-2xl shadow p-4 md:p-6"},
        ...
        React.createElement("ul",{className:"list-disc pl-6 mt-3 space-y-1 text-sm text-slate-700"},
          React.createElement("li",null,"Rounding applies to worked minutes after breaks, to nearest increment."),
          React.createElement("li",null,"Daily OT allocation is per employee per day, chronological by start time."),
          React.createElement("li",null,"Approving a week locks entries; locked entries can't be edited/deleted."),
          React.createElement("li",null,"Import projects CSV headers: code,name. Existing codes are skipped."),
          React.createElement("li",null,"Login uses your backend /auth/login (temporary demo)."),
          React.createElement("li",null,"Next: move data to Neon + real JWT auth; add supervisor notifications.")
        )
      )
    ),

    // Footer
    React.createElement("footer",{className:"max-w-6xl mx-auto px-4 py-8 text-center text-xs text-slate-500"},"Built as a starting point. Let's tailor it to your factory flow.")
  );
}
